...
## 9. Estado em produção e o que a verificação ensinou (2026-09-02)

- PR #32 (Indicação), PR #33 (analytics) e PR #34 (espera pelos scripts) mergeados e em produção no mesmo dia. Envs `CRM_BASE_URL`, `CRM_EXTERNAL_API_KEY` e `INDICACAO_COOKIE_SECRET` na Vercel em Production e Preview. Dimensões `codigo_vendedor` e `formulario` criadas no admin do GA4. Issue explicativa no CRM: profillsbrasil/crm#398.
- Verificado com código real (`OTHAVIO`): 302 com cookie de 30 dias, rodapé com o e-mail e o WhatsApp do vendedor, `indicacao_chegada` com `codigo_vendedor = OTHAVIO` saindo como `POST /g/collect` 204 e visto no Tempo real do GA4 a partir de acessos pelo Chrome do celular. Os CTAs do hero e da ficha de máquina foram conferidos só em desenvolvimento. Lead de teste não foi enviado. Meta não conferido no Gerenciador de Eventos: em produção os browsers automatizados não emitem `facebook.com/tr`; o disparo foi observado no Brave do dono com Shields desligado, sem artefato, e capturado em desenvolvimento.
- Correção da seção 8 depois da revisão: o disparo em `lib/analytics/indicacao.ts` espera `window.gtag` com `dataLayer` e `window.fbq` existirem (tentativa síncrona, depois polling de 250 ms por cerca de 10 s, cada lado independente, desistindo em silêncio). A ordem entre o efeito do Provider e os scripts `afterInteractive` só é garantida na hidratação normal; num render de cliente após falha de hidratação, o efeito roda antes e `sendGAEvent` descartava o evento.
- Limites de verificação: no Pixel o `fbevents.js` descarta o evento no cliente quando a UA é `HeadlessChrome` (nenhuma requisição sai); no GA4 o hit sai com 204 e cai na exclusão automática de bots, que o Google não deixa inspecionar. O Brave do dono bloqueia o `gtag.js` mesmo com Shields desligado. A prova de ingestão só veio de um navegador comum. Guia em `docs/indicacao-verificacao.md`.
- Decisões que a revisão contestou e o dono manteve: chegada por aba (não por clique no link) e HTML estático sem `href` nos contatos comerciais até a hidratação. Chave do CRM não rotacionada, por decisão do dono.
